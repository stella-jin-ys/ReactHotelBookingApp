...
const HotelPage: React.FC = () => {
      ...
      try {
        ...
        if (selectedHotelId) {
          try {
            ...
            console.error(
              "Failed to fetch specific hotel, falling back to all hotels:",
              err
            );
  ...
  if (hotels.length === 0)
    return <div className="container mx-auto px-4 py-8">No hotels found</div>;

  // Parse dates for the AvailabilityComponent
  const checkInDateObj = searchParams.checkInDate
    ? new Date(searchParams.checkInDate)
    : new Date();
  const checkOutDateObj = searchParams.checkOutDate
